...
namespace dev_enum
{
	internal class InfoSender
	{
		...
		{
			StreamWriter log = File.AppendText(string.Format("{0}dev_enum-{1}-added.xml", temp, GetFname()));
		...
		{
			StreamWriter log = File.AppendText(string.Format("{0}dev_enum-{1}-removed.xml", temp, GetFname()));
			log.WriteLine(doc.OuterXml);
			log.Close();
		}
		...
		{
			StreamWriter log = File.AppendText(string.Format("{0}dev_enum-{1}-modified.xml", temp, GetFname()));
			log.WriteLine(doc.OuterXml);
			log.Close();
		}
		...
		{
			StreamWriter log = File.AppendText(string.Format("{0}dev_enum-{1}-heartbeat.xml", temp, GetFname()));
			log.WriteLine(doc.OuterXml);
			log.Close();
		}
